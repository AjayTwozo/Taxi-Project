...
import com.sanmedia.twozo.exceptions.InvalidFareDetailsException;
...
public class BookingDAO {
    ...
    public Driver getDriverAvailability(final Long serviceId, final Long locationId) {
        final String selectDriverQuery = "SELECT com.TaxiProject.driver.id, name, mobile_number, email, registration_number FROM com.TaxiProject.driver " +
                "FULL JOIN service_user ON com.TaxiProject.driver.id != service_user.id WHERE location_id = ? and service_id = ?";
    ...
    public long insertBooking(final Booking booking) {
        final String insertQuery = "INSERT into com.TaxiProject.booking (fare_id, driver_id, total_fare) VALUES (?, ?, ?)";
        final String bookingIdSelectionQuery = "SELECT id FROM com.TaxiProject.booking ORDER BY id DESC LIMIT 1";
    ...
    /**
     * <p>
     *     Updates com.TaxiProject.driver's location in the respective table once the Journey ends.
     * </p>
     *
     * @param locationId {@link Long}, drop Location being updated as Driver's current Location.
     * @return that com.TaxiProject.driver's updated location ID.
     */
    public long updateDriverId(final Long locationId) {
        final String updateLocationQuery = "update com.TaxiProject.driver set location_id = ?";
